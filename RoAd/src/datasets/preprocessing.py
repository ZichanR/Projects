...
def create_semisupervised_setting(labels, normal_classes, outlier_classes, ratio_unlabel, n_pollution):
    """
    ...
    n_known_normal = n_normal - (int((n_normal + n_outlier) * ratio_unlabel) - n_pollution)
    n_unlabeled_normal = int((n_normal + n_outlier) * ratio_unlabel) - n_pollution
    n_unlabeled_outlier = n_pollution
    n_known_outlier = n_outlier-n_pollution
    ...
    perm_outlier = np.random.permutation(n_outlier)

    idx_known_normal = idx_normal[perm_normal[:n_known_normal]].tolist()
    idx_unlabeled_normal = idx_normal[perm_normal[n_known_normal:]].tolist()
    idx_unlabeled_outlier = idx_outlier[perm_outlier[:n_unlabeled_outlier]].tolist()
    idx_known_outlier = idx_outlier[perm_outlier[n_unlabeled_outlier:]].tolist()
